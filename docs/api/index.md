...
This section is generated from the XML documentation comments in the source, so it always matches the shipped release.
...
> [!NOTE]
> `HandlerNotFoundException` ships in `Send0xx.Dispatcher` and can be thrown by either implementation.
> The other exception types ship in `Send0xx.Dispatcher.DependencyInjection`; source generation reports
> their equivalent route and handler-shape failures as compiler diagnostics at build time.

## What is not listed here

The `GenerateDispatcher` and `GenerateDispatcherHandlers` attributes do **not** appear in this reference. The source
generator injects them into your own compilation as internal types rather than shipping them as public API, so there is
no assembly for DocFX to read them from. The generated registration extension methods are likewise emitted into your
project. Both are documented in
