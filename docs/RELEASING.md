...
NuGet publishing runs only for version tags such as `v1.0.0-preview.11`. Before publishing, the workflow verifies that the tag matches the central `Version` in [`Directory.Build.props`](../Directory.Build.props) and pushes an explicit list of package files.
...
```bash
git tag v1.0.0-preview.11
git push origin v1.0.0-preview.11
```
