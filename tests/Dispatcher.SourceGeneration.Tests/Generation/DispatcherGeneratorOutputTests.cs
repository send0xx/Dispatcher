...
public sealed class DispatcherGeneratorOutputTests
{
    ...
    public void Generates_dispatcher_for_internal_handlers()
    {
        ...
        Assert.Contains("\n            [typeof(global::TestQuery)]", generated, StringComparison.Ordinal);
        Assert.Contains("\n                static (dispatcher, message, token)", generated, StringComparison.Ordinal);
    ...
    public void Generates_conditional_telemetry_dispatcher_and_exception_events()
    {
        ...
        Assert.Contains("activity.AddEvent(new global::System.Diagnostics.ActivityEvent", dispatcher,
            StringComparison.Ordinal);
    ...
    public void Generates_key_free_open_notification_registration_and_dispatch_plan()
    {
        ...
        Assert.Contains("\n            if (", dispatcherRegistration, StringComparison.Ordinal);
        Assert.Contains(
            "\n            {\n                continue;\n            }",
    ...
    [Fact]
    public void Omits_unmatched_open_notification_handler_from_dispatch_runtime()
    {
        const string source = """
                              using Dispatcher;
                              using Dispatcher.SourceGeneration;
                              ...
                              internal interface IAuditNotification : INotification;
                              internal sealed class AuditHandler<TNotification> : INotificationHandler<TNotification>
                                  where TNotification : IAuditNotification
                              {
                                  public ValueTask HandleAsync(
                                      TNotification notification,
                                      CancellationToken cancellationToken) => ValueTask.CompletedTask;
        ...
        var registration = Assert.Single(result.GeneratedTrees.Where(tree =>
            tree.ToString().Contains(
                "public static class GeneratedDispatcherServiceCollectionExtensions",
                StringComparison.Ordinal))).ToString();
        Assert.DoesNotContain("global__AuditHandler", dispatcher, StringComparison.Ordinal);
        Assert.DoesNotContain("global__AuditHandler", registration, StringComparison.Ordinal);
        Assert.Empty(result.OutputCompilation.GetDiagnostics(TestContext.Current.CancellationToken)
            .Where(diagnostic => diagnostic.Severity == DiagnosticSeverity.Error));
    }
